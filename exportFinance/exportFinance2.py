...
# def load1():
#     load("2018-11-01 00:00:00", "2018-12-01 00:00:00");
# threading.Thread(target=load1,name=("thread-%s",1)).start();
#
# def load2():
#     load("2018-12-01 00:00:00", "2019-01-01 00:00:00");
# threading.Thread(target=load2,name=("thread-%s",2)).start();
#
# def load3():
#     load("2019-01-01 00:00:00", "2019-02-01 00:00:00");
# threading.Thread(target=load3,name=("thread-%s",3)).start();
#
# def load4():
#     load("2019-02-01 00:00:00", "2019-04-01 00:00:00");
# threading.Thread(target=load4,name=("thread-%s",4)).start();

def load1():
    load("2018-12-01 00:00:00", "2018-12-10 00:00:00");
threading.Thread(target=load1,name=("thread-%s",1)).start();

def load2():
    load("2018-12-10 00:00:00", "2018-12-15 00:00:00");
threading.Thread(target=load2,name=("thread-%s",2)).start();

def load3():
    load("2018-12-15 00:00:00", "2018-12-20 00:00:00");
threading.Thread(target=load3,name=("thread-%s",3)).start();

def load4():
    load("2018-12-20 00:00:00", "2018-12-25 00:00:00");
threading.Thread(target=load4,name=("thread-%s",4)).start();

def load5():
    load("2018-12-25 00:00:00", "2019-01-01 00:00:00");
threading.Thread(target=load5,name=("thread-%s",4)).start();

def load6():
    load("2019-01-01 00:00:00", "2019-02-01 00:00:00");
threading.Thread(target=load6,name=("thread-%s",3)).start();

def load7():
    load("2019-02-01 00:00:00", "2019-04-01 00:00:00");
threading.Thread(target=load7,name=("thread-%s",4)).start();
